...
(function() {
		...
		function allowAnywhereFoldCaseAndRenumberGroups(regex) {
			...
			for (var i = 0, groupIndex = 0; i < n; ++i) {
				...
				if (p === '(') {
					...
					var decimalValue = Number(p.substring(1));
					if (decimalValue) {
	...
	function createSimpleLexer(shortcutStylePatterns, fallthroughStylePatterns) {
		...
		var decorate = function(job) {
			const {sourceCode} = job;
			const {basePos} = job;
			const {sourceNode} = job;
	...
	function sourceDecorator(options) {
		...
		const keywords = String(options.keywords).replace(/^ | $/g, '');
		if (keywords.length) {
			fallthroughStylePatterns.push([
				PR_KEYWORD,
		...
		function breakAfter(lineEndNode) {
			...
			function breakLeftOf(limit, copy) {
				...
				if (parent) {
					...
					parentClone.append(rightSide);
					for (let sibling = next; sibling; sibling = next) {
						next = sibling.nextSibling;
						parentClone.append(sibling);
					}
				}
	...
	function recombineTagsAndDecorations(job) {
		...
		isIE8OrEarlier = isIE8OrEarlier && Number(isIE8OrEarlier[1]) <= 8;
		const newlineRe = /\n/g;
	...
	function $prettyPrint(opt_whenDone, opt_root) {
		...
		for (const element of codeSegments) {
			for (let j = 0, n = element.length; j < n; ++j) {
					...
					if (!nested) {
						...
						if (preformattedTagNameRe.test(cs.tagName)) {
							...
							const whitespace = currentStyle
								...
								: 0;
							preformatted = whitespace && whitespace.substring(0, 3) === 'pre';
						}

						// Look for a class like linenums or linenums:<n> where <n> is the
						// 1-indexed number of the first line.
						let lineNums = attrs.linenums;
						if (!(lineNums = lineNums === 'true' || Number(lineNums))) {
